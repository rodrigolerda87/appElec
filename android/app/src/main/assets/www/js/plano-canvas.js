...
export class PlanoCanvas {
  ...
  // --- Coordenadas: de evento de puntero a píxeles internos del canvas ---
  ...
  _onPointerDown(e) {
    ...
    if (this.mode === 'calibrar') {
      this._calibPoints.push(p);
      this.redraw();
      if (this._calibPoints.length === 2) {
        const dPx = dist(this._calibPoints[0], this._calibPoints[1]);
        this.cb.onCalibration && this.cb.onCalibration(dPx, () => { this._calibPoints = []; this.redraw(); });
      }
      return;
    }

    if (this.mode === 'ambiente') {
      // Si se toca cerca del primer punto (y hay al menos 3), cierra el polígono.
      if (this._activeRoomPoints.length >= 3 && dist(p, this._activeRoomPoints[0]) < 14) {
        this._closeRoom();
        return;
      }
      this._activeRoomPoints.push(p);
      this.redraw();
      return;
    }

    if (this.mode === 'canalizacion') {
      this._activeCurvePoints.push(p);
      this.redraw();
      return;
    }
  }
  ...
  // tocar exacto el primer punto (dedos grandes, plano chico).
  ...
  redraw() {
    ...
    // Canalizaciones
    for (const c of this.canalizaciones) this._drawCurve(c.points, c.color, 3);
    if (this._activeCurvePoints.length) this._drawCurve(this._activeCurvePoints, this.currentColor, 3, true);
  ...
  snapshotDataUrl() { return this.canvas.toDataURL('image/jpeg', 0.85); }
